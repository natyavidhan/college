...
for i in range(n):
    data = input(f"Enter AT, BT and Priority (space separated): ")
    at, bt, priority = map(int, data.split())
    processes.append({"pid": i + 1, "arrival_time": at, "burst_time": bt, "priority": priority})
...
for pid in sorted(process_stats.keys()):
    ...
    total_tat += tat
    total_wt += wt
    print(f"P{pid}\t{at}\t{bt}\t{priority}\t\t{ct}\t{tat}\t{wt}")
